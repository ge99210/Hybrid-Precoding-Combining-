...
SNR_set = -30:5:0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% GSSA input parameters (fixed to)
Tmax = 100;
EPS = 1e-4;
M = 7;
mu_v(M) = 2.5;
...
% Initialization for the Chineses' algorithm
eps = 0.1;
delta = 0.1;
K_u = 100;
...
for SNR_index = 1:length(SNR_set)
    ...
    for i=1:N_iter   
        ...
        switch choice
        case 1 % mmWave channel model v2 - Nc clusters Np paths per cluster 2D
        % Channel checked and verified 6/7/2019
        ...
        case 2 % Rician mmWave channel model v2 - Nc clusters Np paths per cluster 2D
            [H, Phi_AOD, Phi_AOA, Alpha] =...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
        % HP via OMP
        tic;
        [F_R_md, F_B_md] = hybrid_omp(V1, A_t, Ns, Mt);
        time_OMP = toc;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
        % HP via Matrix Decomposition
        [U_F,S_F,V_F] = svd(V1);
        Ph_0 = angle(U_F*S_F);
        F_R_0 = exp(1j*Ph_0)/sqrt(Nt);
        tic;
        [F_R_md, F_B_md] = alternate_approximation_hybrid_precoder(V1, F_R_0, eps, delta, K_u); % try the adaptive threshold
        time_md = toc;
        F_md = F_R_md*F_B_md;
...
hl = legend('Fully Digital','OMP','MD','SAGS','Location','Northwest');
